    ...
    try:
        with SyncCrazyflie(radio_uri, cf=Crazyflie(rw_cache="./cache")) as scf:
            ...
            while True:
                ...
                # Preprocess the frame for the obstacle model
                preprocessed_frame = preprocess_image(frame)
                model_prediction = obstacle_model.predict(preprocessed_frame)[0][0]  # Get prediction
                ...
                # Combine obstacle model prediction with contour detection
                if model_prediction > 0.5 or (obstacle_detected and abs(contour_x - frame.shape[1] // 2) < 100):
